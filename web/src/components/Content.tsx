    ...
    return (
        <div className="h-full p-2 md:p-4 flex flex-col">
            <div className="relative flex-1 flex flex-col min-h-0">
                ...
                <div 
                    className="absolute -top-3.5 left-4 px-2 py-0.5 border border-ui-border-ui text-ui-item-folder z-30 text-sm transition-colors duration-300 font-bold"
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar rounded-ui z-10 flex transition-all duration-300">
                    {/* Line Numbers */}
                    <div className="hidden md:flex flex-col items-end py-8 pl-4 text-gray-500/50 font-mono text-sm select-none bg-bg-primary/50 min-h-full">
                        <div className="border-r border-ui-border-ui/10 pr-2">
                            {Array.from({ length: Math.max(lineCount, 50) }).map((_, i) => (
                                <div key={i} className="leading-relaxed h-[24px]">{i + 1}</div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-3 md:p-6">
                        ...
                        {file.frontmatter && Object.keys(file.frontmatter).length > 0 && (
                            <div className="mb-8 font-mono text-[16px]">
                                        ...
                                        {Array.isArray(value) ? (
                                            <div className="flex flex-wrap gap-2 mt-1">
                                                {value.map((item: string, i: number) => (
                                                    <span 
                                                        key={i} 
                                                        className="text-text-body text-sm border border-ui-link px-2 py-0.5 rounded-md bg-ui-link/10"
                                                    >
                                                        {item}
                                                    </span>
                                                ))}
                            ...
                            <div className="mb-12 font-mono ml-2">
                                {file.frontmatter.timeline.map((item: any, index: number) => (
                                    <div key={index} className="relative pl-8 pb-8 border-l border-ui-border-ui last:border-l-0 last:pb-0">
                                        ...
                                        <div className="text-sm text-ui-highlight mb-1">{item.date}</div>
                                        <h3 className="text-lg text-text-heading font-bold">{item.title}</h3>
                                        <div className="text-text-body">{item.institution}</div>
                                        {item.location && <div className="text-text-body text-sm opacity-75 mt-1">{item.location}</div>}
                                        {item.details && Array.isArray(item.details) && (
                                            <ul className="list-disc list-inside mt-2 text-text-body text-sm opacity-90">
                        ...
                        <div className="prose prose-invert prose-p:text-text-body prose-headings:font-normal max-w-none">
                            <ReactMarkdown 
                                ...
                                components={{
                                    ...
                                    h1: ({node, ...props}) => <h1 {...props} className="text-[24px] text-text-heading mt-8 mb-4 font-mono"><span className="opacity-50 mr-2">#</span>{props.children}</h1>,
                                    h2: ({node, ...props}) => <h2 {...props} className="text-[24px] text-text-heading mt-8 mb-4 font-mono"><span className="opacity-50 mr-2">##</span>{props.children}</h2>,
                                    h3: ({node, ...props}) => <h3 {...props} className="text-[20px] text-text-heading mt-6 mb-3 font-mono"><span className="opacity-50 mr-2">###</span>{props.children}</h3>,
                                    p: ({node, ...props}) => <p {...props} className="mb-4 leading-relaxed text-text-body font-mono text-[16px]" />,
                                    ul: ({node, ...props}) => <ul {...props} className="list-disc list-inside mb-4 space-y-1 text-text-body font-mono text-[16px]" />,
                        ...
                        {file.frontmatter?.cta && (
                            <div className="mt-12 p-6 border border-ui-border-ui rounded-ui bg-bg-primary/30 backdrop-blur-sm">
